...
- Universe 建構（v3.3.2 修法，舊 V0.23）改 per-rebalance-date PIT-safe filter，杜絕用 future close 過濾
...
- **FDR Benjamini-Hochberg** multiple-testing 校正（v2.0 5 因子 pre-registered → m=5；v3.x 的 3 因子 2026-05-11 補測 single IC，不在 m=5 內）
...
- D-A 預先 disqualify（per v2.1 的 D6 OOS IR 0.0058 / 99.4% collapse），不在 sweep 範圍
...
```
src/
├── portfolio/tw_stock.py      核心選股（~1900 LOC，目前單檔最大，已列為 v4.x 前拆檔 candidate）— _analyze_symbol → _rank_analyses → _select_positions
...
├── data/
│   ├── finmind.py             FinMind API + pickle cache（v3.3.1 transient classification）
...
scripts/                       執行 CLI
├── d_cell_sweep_v7_real.py       v3.3 的 18-cell sweep runner（檔名保留 v7）
...
├── run_factor_ic.py              v2.0 的 5 因子 IC CLI（full-universe Spearman IC + DSR / FDR / bootstrap / permutation）
├── run_phase_d_factor_ic.py      v3.x 的 3 因子 IC CLI（quality_v3 / industry_momentum / idio_vol_max；2026-05-11 補測）
...
└── cache_fill_new_factors.py     v3.x 新因子 cache fill（含 --seed-issued-capital）
...
| 代號 | 是什麼 |
|---|---|
| **D1_v2** | v2.1（舊 Phase A2）的雙因子組合 = 52W 高接近度 50% + PEAD 50%（IS IR 0.92 → OOS 0.0058 collapse 那個）|
| **D-A … D-G** | v3.x（Phase D）候選因子組合（D-B…D-G 共 6 個進 sweep；D-A 已預先 disqualify，不在 sweep）|
...
- **頁 2 因子 IC 測試** — 8 因子單獨 IC 主表（v2.0 的 5 + v3.x 的 3，2026-05-11 補測；FDR m=5 只跑 v2.0）+ 5×5 Spearman correlation（v2.0 only）+ 進階分析
- **頁 3 雙因子回測** — D1_v2（52W 50% + PEAD 50%）IS+OOS 累積報酬 + 12 metrics 對照（IR 0.92 → 0.0058 collapse）
- **頁 4 18 種策略最終 sweep** — v3.3 的 6×3 cells heatmap + L1-L6 詳表 + 月超額時序
- **頁 5 為什麼相信這個 NO-GO** — 雙重否定 triangulation：揭穿 Overfit + Bootstrap CI 數學

依賴：streamlit + plotly + pandas-ta（已在 requirements.txt）。

### 5. 跑 v3.3 的 18-cell sweep
...
# v3.x 的 3 因子（2026-05-11 補測；用 CellSweepContext 資料源 + 自動補 enrichment 診斷）
...
| [reports/factor_ic/phase_a1_summary.md](reports/factor_ic/phase_a1_summary.md) | v2.0 的 5 因子綜合結論 |
...
## 18 種策略最終結果一覽（v3.3 的 18-cell sweep）
...
2. **n_trials=18 DSR 診斷**：18 個假設一起測，單一通過 90% 機率會被壓縮；但 v3.3 的 binding NO-GO 來自 L1-L6 hard gates，不是只因 DSR
...
v2.0 的 5 因子 + v3.3 的 18 cells 全 NO-GO 之後，校正出來的結論是：**「台股月頻 long-only、60 個月樣本下找穩定 factor edge 的 base rate 很低」**——不是哪個 evaluator 寫太嚴，是這個 setup 本身的統計力天花板就在那（60 monthly obs × block_len=3 → effective n ≈ 20）。
...
## 結論之後：如果要讓它變 GO，下一步會怎麼做（v4.x hypothesis，不是承諾）

v4.x 不會是「微調 v3.3 的權重把它調到過關」（那是 p-hacking、是 silent_bug），而是**從一開始就改 setup**。下表是「哪個方向 / 為什麼可能讓 NO-GO 變 GO / 誠實的 caveat」——重點是「方向」不是「保證」，任何一條都可能再次 NO-GO，那才是誠實的 prior：
...
→ v4.x 真開跑前會先寫一份新 pre-registration（樣本窗 / universe / 引擎路徑 / 成本 / gates / max trials 事前鎖定），跟 v3.3 一樣**不允許事後 retune**。詳見 [docs/research-findings.md](docs/research-findings.md) 的「下一步（A-then-B）」段。
