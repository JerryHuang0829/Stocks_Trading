...
### Codex Round 1（pre-run）→ Round 2 → Round 3 verdict 鏈
...
### V0.26 NetIncome 黑天鵝（Codex Round 2 found）
...
### Codex R25-final 與 Claude 獨立驗證雙重 CONFIRM-NO-GO

User 在收 Codex R25-final 後指示「請你實際驗證 不要依賴codex」。Claude 直接 read cell_summary.json + 跑 _verify_qv3_period.py 獨立驗證：
...
Claude 給 Codex 的 R25-final prompt 表格手算錯 4 cell（D-B|12, D-B|16, D-C|8, D-C|16），Codex 比對 evaluator JSON 抓出來。**這是 prompt 手填錯，不是 evaluator bug**。Memory `feedback_silent_bugs.md` 已增補此 case。
...
- 更新 CLAUDE.md「目前狀態」section
- 追加本 changelog 區塊
- 追加 策略研究.md Phase D v7 結論
- 重寫 Claude-Prompt.md 為 next-session = await user A/B/C
...
- Tag emit shell：`emit_phase_d_v7_complete_tag_command()` 返回 `git tag phase-d-v7-complete <SHA>` 命令字串（**deliberately NOT auto-tag**，user 自跑 git tag command per CLAUDE.md "git safety protocol"）
...
**Path C**: 直接進 S8 sole_survivor lock + commit phase-d-v7-complete tag (基於 S6 stub aggregate function)；S6.1 + S7 真實 run + 真 cell_summary 由 user 端產出後再 retrigger Codex R25-final
...
## 2026-05-05：Plan v7.1 Reframe — R25-mid Codex audit 5 P0 修法 (B' 1 commit 整包)

R25-mid Codex audit verdict = **GO-WITH-CAVEATS（5 P0 必修）**。Codex 親跑 code + targeted tests，verdict 抓出 4 P0 設計層問題；user (Claude) 親自驗證確認 4 P0 全 valid + 加 1 P0 (pre-commit #8 wording 模糊)。**candidate pool 定義已被污染** — 即使數值計算正確，會回答錯誤的實驗問題。User 拍板 Plan v7.1 Reframe (B' 1 commit 整包) ~5.5 hr 工程修畢進 S5 ready 狀態。

### Codex 5 P0 親自驗證結論

| P0 # | Codex 抓 | 親自驗證 evidence |
|------|---------|-----------------|
| **P0-1** | D-C 50/50 ≡ D-A 50/50 D1_v2 design (pre-disqualified) | H_d_v6:177 D-C "50/50 D1_v2 baseline" + H_d_v6:207 D-A "(52W + PEAD 50/50, weight = D1_v2 design)" + D-C.yaml 0.50/0.50 = 數學 ≡ D-A; module-level string-only Assertion 2 cannot catch ✅ Codex 對的 |
| **P0-2** | D-D 含 revenue_momentum_v2 違反 pre-commit #8 | H_d_v6:196 #8 "Revenue_v2 exclusion cannot be reversed" + H_d_v6:178 D-D "high_proximity + pead_eps + margin_short + revenue_momentum_v2" + D-D.yaml revenue_momentum_v2: 0.21 = H_d_v6 內部矛盾 ✅ Codex 對的 |
| **P0-3** | Assertion 2/3 既有 tests 只測 string，不抓 composition equivalence | tests/test_d_cell_sweep_v7.py:36-50 全測 string `"D-A"` literal + typo variants；沒測「factor dict equals D-A composition」 ✅ Codex 對的 |
| **P0-4** | active_corr docstring/code mismatch | active_correlation.py:35-37 docstring "Raises if non-aligned indexes" 但 line 52-57 code 只 `len()` check；同 length 不同 dates → silent 算錯 ✅ Codex 對的 |
...
- 對應 H_d_v6:177 D-C row 改 description; spec_source 加 V0.14 R25-mid Codex audit P0-1 fix ref
...
- 新加段 §"V0.14 Codex R25-mid Audit P0 fix log" before §"Sign-off"
- Audit chain 加 V0.14 entry
- R16/R17 risk register 加 (D-C redesign IR-weighted impact / D-D 與 D-B 區分機制)

#### Fix-7 (acknowledge 不修): Codex Windows pytest tmp permission
- Codex 在 Windows 撞 9 PermissionError tmp tests — 環境問題非 logic
...
- audit_doc_drift LATEST_AUDIT_ROUND R24 (Codex R25-mid v2 confirmation 後才 bump R25)
...
- **R25-mid v2 confirmation (optional)**: user 決定是否再送 Codex 確認 v7.1 reframe fix；不主動再送
- 等 user 核可才開工 S5
...
**S4 完成觸發 🚨 R25-mid Codex audit checkpoint** (HANDOFF Section 0 + Plan v7 鎖定)。
...
- audit_doc_drift LATEST_AUDIT_ROUND R24 (Codex R25-mid 完才 bump)
...
### 🚨 R25-mid Codex audit checkpoint

S4 完成觸發 R25-mid Codex audit per HANDOFF Section 0 + Plan v7 鎖定:
1. 我**不主動 spawn Codex** — user 手動送
2. user 修 `Codex-Prompt.md` anchor (從 v7-baseline `d55d4ea` 改為 S4 完成 commit) + Mission focus (從 R25-final 改為 R25-mid 設計層 audit)
...
**S4 完 → 🚨 R25-mid Codex audit checkpoint** (user 手動送 Codex)
...
V1.1 原 spec line 267 deliverable: 「H_d_v6 §"6 Hard Reject Criteria" 補 L1 / L2 numerical justification」。V1.1a V0.13 已 inline short rationale（line 33-36）；V1.1c 擴充為正式段 + 3 table form 完整數值驗算 + Conclusion，per V1.1 原 spec deliverable + 防 R25-mid Codex audit「numerical justification 不足」P0 attack。
...
- audit_doc_drift LATEST_AUDIT_ROUND R24（待 Codex R25-mid 完才 bump）
- Plan v7 hypothesis lock
...
- Batch 1 ~32 MB 磁碟雜訊（pytest/codex scratch / tests/_tmp / data 進度修復檔 / logs 舊檔；全 .gitignore'd）
...
## 2026-05-04：Pro Validation Sprint Phase A-J（另一 Claude session 平行做）
...
- SOP 6 步完整做：
  ...
  - Step 5 Self-attack：3 Codex-level 挑戰點（跨產業 pool 語意 / double-count / threshold 放寬）皆有防禦
...
- Codex audit chain：R18 後結束；R19 不排程

### Gitignore 更新（本次）

取消 ignore：
- `CLAUDE.md` / `HANDOFF.md` / `Claude-Prompt.md` / `Codex-Prompt.md` / `策略研究.md` / `優化紀錄.md` / `教學進度.md`
- `.claude/*` full unignore（含 agents + commands + settings.json + sop-*）
...
- 刪 9 個暫存目錄（`.codex_pytest_tmp` / `.pytest_tmp` / `.pytest_local_thresholds` / `.pytest_r4_*` / `tmpwyew0fr2` / `verify_tmp_r4_*` / `verify_tmp_r5_manual`）
...
- 4 新 helper：
  - `_safe_fetch` re-raise `_BacktestCacheMissError`（Codex Round 14 P1-1 鎖定）
  - `_bulk_fetch_latest_market_value` 用 `fetch_market_value(days=10)` 無 symbol arg + groupby tail(1)（Codex Round 14 P0-1 鎖定）
  - `_load_issued_capital_dict` 顯式 str/float dtype cast + coverage warning（Codex Round 14 P0-2 鎖定）
...
- `tests/test_step2_factor_integration.py` 23 tests：
  ...
  - Group C integration + Codex Round 14 P0 regression (9)：bulk fetch_market_value no-symbol-arg / groupby latest / issued dtype cast / coverage warning / _safe_fetch re-raise / _safe_fetch swallows others / available_metrics 5 new / DEFAULT config / profile config
...
- **2 bug 自揭自修**（Codex Round 14 plan audit 未抓，因為只審 plan 不審 code）：
...
1. **Plan audit 抓不到 code bug**：Codex Round 14 只審 plan 層，無法預測 `_DataSlicer.fetch_ohlcv` 簽章細節；post-execute smoke 才是撈 bug 最後一層
...
4. **Step 5 前 needs more limit evaluation**：`limit=500` 在 2024-07 起的 smoke 夠用，但 walk-forward 2020 IS 時 cache 歷史 <500 day 可能不夠 → Codex Round 16 重點驗
...
Step 3   ⏸ 待 Codex Round 16 post-execute audit（user 手動跑 external）
...
## 2026-04-21：Phase A2 Step 1.5 架構重整（Codex Round 14-plan-review → Step 1.5 → Round 15）

### 背景：Codex Round 14-plan-review audit 判 No-Go

Claude 寫 Phase A2 Step 2 plan（5 factor 整合，`phase-a2-typed-pizza.md` ~720 行）。經過：
1. Claude subagent 扮 Codex 自審 → 抓 3 P0 + 6 P1（API 簽章錯用 / dtype / hardcode / SOP 缺 T4/T5 / stub 不全 / smoke 門檻）→ 全吸收進 plan
2. 雙視角二次 review → 又抓 A1-A5 量化主管視角 + B1-B5 投資人視角 共 10 個 gap
3. **真 Codex external audit（ChatGPT）**判 No-Go，抓 Claude family 都漏的 2 個 P0：
...
### 未 mitigated 挑戰（Codex Round 15 post-execute audit 對象）
...
```
Claude 寫 Step 2 plan
  ↓
Claude subagent Round 14-mock → 3 P0 + 6 P1 吸收
  ↓
雙視角二次 review → 10 gap
  ↓
真 Codex Round 14-plan-review → No-Go，抓 subagent 漏的 2 P0
  ↓
用戶暫停 Step 2，批准 Step 1.5
  ↓
Step 1.5 執行完 3 commits / 369 passed
  ↓
交棒 Codex Round 15 post-execute audit
```

### 關鍵教訓

1. **Claude family 血統共盲點確實存在**：subagent mock audit 抓 3 P0 但仍漏架構層的 duplicate loop + silent renormalize；真 external Codex third-party audit 不能省
...
- Stage 4 自驗 1300/1300 match → **被 Codex R12 打破**（survivor bias）

**Codex R12 揭發**：用 live-vs-cache 實測，2019-06-17 TPEX snapshot 抓到 4 支 sym（1597/1795/3092/4736）仍有 swap 殘留，原因：v1 filter 用 `stock_info.type=='tpex'`（811 syms），漏掉**轉板股**（歷史 TPEX → 現 TWSE，e.g., 1597 直得）。
...
**Migration script**：`scripts/migration_r11_1_tpex_swap_fix.py` + `scripts/migration_r11_1_plan_v2.json`（149KB，tracked），idempotent，新機器可重現 v2 patch state。Codex R12 schema KeyError → `_normalize_plan()` adapter 修完。

### R13：DSR 語義修（commit `cfd676f`）

**Codex R13 holistic audit 評 B+** 揭發：`deflated_sharpe_ratio` 函式回 `stats.norm.cdf(z)` 卻被 code/doc/CLI 誤稱 'p-value'。語義完全顛倒：
...
- `.claude/commands/factor-ic.md:60,77`
- `.claude/agents/methodology-auditor.md:35,57`
...
R13 DSR 事件後，學到 Codex holistic audit 比 line-by-line 審計更能抓結構 bug。

---

## 2026-04-16：Round 3 Codex 驗證 + follow-up
...
## 🧹 2026-04-16：三輪 Codex 架構審計 — 8 個 silent-degradation bug 修完

### 審計動機

Pre-filter / timezone 兩個 bug 修完後，讓 Codex 從架構層面反覆審計，目標：把所有**不 raise、不警告、但讓 KPI 失真**的路徑全部揪出來。三輪下來共 8 個 bug，都已修好並加 regression tests。
