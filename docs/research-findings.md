...
從 external audit Round 1 pre-run audit NO-GO 開始，逐個修法到 V0.26：
...
### 雙重驗證：R25-final + self-audit 獨立 read JSON

User 在收 external audit CONFIRM-NO-GO 後指示「請你實際驗證 不要依賴外部 audit」。self-audit 直接 read cell_summary.json + 跑 `scripts/_verify_qv3_period.py`、`scripts/_verify_tsmc_schema.py` 獨立確認：
...
→ 結論：**CONFIRM-NO-GO 不是 external audit 過度謹慎，是真實沒過。**
...
- R25 round 1-3 + R25-final 4 輪 audit 累積 ~50 個攻擊角度全答完
- self-audit 獨立驗證 SOP 練到位（不依賴 external audit 也能交付）

**負面教訓**：
- Phase A1 5 因子 + Phase D 6 candidates × 3 top_n 全 NO-GO → **台股 monthly freq long-only 60-month sample 找穩定 edge 的 prior 應再降低**
- 給 external reviewer 的 prompt 表格手算錯 4 cell（silent bug pattern #2 + #3：未實測就寫文件 + 修法無 grep sweep）
...
### 三方審計結論（self-audit 診斷 + external audit fact-check + 獨立 self-audit 審計）
...
**總時間**：4 週（Phase A1 4 週時程詳見 `review-prompt.md`）
**關鍵變更（2026-04-16 晚）**：
...
Round 3 external audit follow-up 修復完成後，重跑 walk-forward：
...
### external audit 對 P3 結論的補充（2026-03-31）

- ✅ 同意：波動率加權削弱動能 alpha 來源（邏輯成立）
- ✅ 同意：品質因子稀釋動能（artifact 支持）
- ⚠️ 但「三因子已是最佳」當時講太滿；更準確應說：「P3 測試的兩方向中三因子較佳」
- 2026-04-16 事後：**整個三因子架構已確認無 edge**，此 external audit 補充仍適用，但三因子本身就該汰換
...
> 詳細 Phase A1 4 週時程 + 5 因子實作規格 → **`review-prompt.md`**
