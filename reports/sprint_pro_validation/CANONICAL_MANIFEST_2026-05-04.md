...
| J | Multi-perspective + Codex pre-audit | pending | next |
...
4. **Codex 上輪 audit 主結論「IC JSON n=71，文件引用 phase_a1_summary 的 n=59 過時」全 verified**
...
|---|----------------|--------------|------------------|--------|
| **B1** | L3 TE [0.04, 0.12] → **[0.10, 0.30]** | Codex-Prompt.md:120,161,227 | D1_v2 IS=0.2367 / OOS=0.2231 in v5 range ✅ | ✅ |
| **B2** | IC source phase_a1_summary (n=59) → `reports/factor_ic/*_ic.json` (n=71) | Codex-Prompt.md:121,228 | 5 IC JSON `n=71` reproduced bit-exact | ✅ |
| **B3** | 跨頻 infra (v5 monthly only, v6 預留) | Codex-Prompt.md:122,229 | tw_stock.py:196-197 monthly-only enforced | ✅ |
| **B4** | Cost 1.14% → **0.67% × one-way turnover** | Codex-Prompt.md:123,230 | engine.py:467-472 公式 verified；composite_backtest.py:47 待 v5 Session 1 修 | ✅ |
| **B5** | A5 attacker → D6 hard disqualifier | Codex-Prompt.md:124,231 | D1_v2 IR 0.94 → 0.0058 OOS 觸發 D6 ✅ | ✅ |
...
**1.14% 字面**全 repo grep 命中只有 `Codex-Prompt.md:123,230`（v5 spec **自身對 v4 wrong number 的引用**），不是 active code。
...
**Drift source identified**: `scripts/composite_backtest.py:47` 寫死 `TW_ROUND_TRIP_COST_BPS = 57.0`，與 engine.py 的 `0.47% + 0.2%` 雙模型分歧。Plan v5 Session 1 必修（已 flag in Codex-Prompt.md:90）。

---

## 7. Cross-Frequency Support Audit（Phase F）

`tw_stock.py:196-197` 確認 monthly hardcoded（intentional per v5 pre-commit rule #6 `Codex-Prompt.md:185`）：
```python
...
**Mutation meta-verification**: 內聯 Python 模擬 cutoff bypass，確認 assert 會 catch (`AssertionError: FORWARD LEAK: future row open=999 leaked`)。**這是 codex 上輪 smoke check 缺的一層**。
...
### ⚠️ 最重要 caveat — Cache Reproducibility (Codex Q8.1 attack 必須誠實標示)
...
**Codex 上輪 audit 我的修正**：
- ✅ Codex「459 tests」claim **是對的**（我 explore agent 數錯為 453；實測 459-462）
- ✅ Codex「IC JSON n=71」claim 全對
- ✅ Codex「D1_v2 TE 0.2367/0.2231」claim 全對
- ⚠️ Codex「1.14% 文件 vs 0.67% engine 不一致」claim 是 **misleading 不是 false**——指向 composite_backtest.py:47 雙模型 drift，根因不是文件 typo
- ⚠️ Codex「PIT slicer no leakage」是 smoke pass，**Phase C 用 4 個 mutation test 升級為實證 verified**
