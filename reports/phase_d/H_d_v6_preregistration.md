...
**Plan version**: v6.2 (V0.14 + R25-mid Codex audit 5 P0 fix; supersedes v6.1 V0.13 + v6.0 R24 5 P0 + 7 design issues fixed)
**Commit hash anchor**: `phase-d-v6-baseline` tag (created by Phase 0 V0.7 — see `v6_validation_manifest.md` for resolved hash)
**Repo**: `<repo_root>`
**Codex audit chain**: R24 (Plan v5 NO-GO — see R24_resolution.md §"Scope correction") → R25 (planned, post-Phase 2)
...
| **L5** A1 gate (3 sub-conditions, all must pass) | (a) active corr ≤ 0.50 (b) TE ≥ 0.10 (c) beta-adj α t > 1.5 | **same as v5** | Active-share + TE + statistical significance combo (originally Codex R20 finding F1). **V1.2 binding**: active_corr function implementation locked to Phase 2 Session 5 (see §"L5 active_corr binding (V1.2 lock)" below). |
...
**v6 降標 mathematical reasoning** (per Codex R24):
...
| **D-C** | high_proximity (52W) + pead_eps | **PEAD-weighted 40/60 (V0.14: NOT 50/50 — D-A ≡ 50/50 D1_v2 design pre-disqualified per R25-mid Codex audit P0-1)** | None |
| **D-D** | high_proximity (52W) + pead_eps + margin_short | **3-factor IR-weighted normalize 34/36/30 (V0.14: revenue_momentum_v2 移除 per pre-commit #8 V0.14 clarify; R25-mid Codex audit P0-2)** | None |
...
8. **Factor exclusion semantics (V0.14 clarify per R25-mid Codex audit P0-2/P0-5)**:
...
|---|---|---|
| `_DataSlicer` PIT 截斷 | 每 factor 模組內部 `as_of` filter（high_proximity.py:74 / pead_eps.py:45 / quality_v3.py:127 / industry_momentum.py:91 / idio_vol_max.py:120 全 enforce shift=1）| Codex pre-run audit 第三部分逐因子驗 ✓ |
...
| Universe MIN_PRICE PIT 過濾 | V0.23 `_is_above_min_price_at()` per-rebal-date PIT-safe filter（取代 V0.13 forward-looking `df["close"].mean()` bug）| Codex 2026-05-06 audit P0-2 修法 |
| drift-aware daily return (P4.6) | **不適用** — V0.15 用 month-end-to-month-end gross return；caveat: vol 略低估 ~5-10%（無 mid-period drift）；但 IS IR 測量上 conservative（不會 inflate alpha）| Codex pre-run audit 第四部分 4.1 已 flag |
...
V0.15 amend 採取「重現 BacktestEngine 提供的 6 個保證」策略 — 透過 6 因子 module 自身 PIT enforce + caller 層級 dividend/split/MIN_PRICE 處理，達成 same-rigor，工程量低且可審查（Codex pre-run audit 已逐項驗證 C1-C6）。
...
See Plan v6 R1–R13 (in `<user_home>\.claude\plans\pro-plan-shimmering-pizza.md`). 13 known risks with mitigations; R6 (D-A pre-disqualification ≠ hypothesis editing) and R12 (L6 80% may still kill majority) are the most consequential for v6.
...
## V0.14 Codex R25-mid Audit P0 fix log (2026-05-05)

R25-mid Codex audit verdict = **GO-WITH-CAVEATS（5 P0 必修）**。Codex 親跑 code + targeted tests + 親自驗證；user (Claude) 親自 cross-check 5 P0 全 valid。v7.1 reframe 修法在 V0.14 amend：

| P0 # | Codex 抓 | Fix in V0.14 |
|------|---------|-------------|
...
- **V0.14 §"Factor exclusion semantics" (pre-commit #8) + §"D-A composition Assertion 2 強化" + §"active_corr index alignment fix"** — R25-mid Codex audit P0 closeout
...
**Pre-registered by**: Claude Opus 4.7 + <user>
**Audit chain**: R24 NO-GO (5 P0 + 7 設計，scope per R24_resolution.md §"Scope correction") → v6 + Sprint Phase A-J integration → v7 V0.8-V0.12 closeout → V0.13 (Phase 1 V1.1a 4 P0 spec lock from R25-mid Pro Review in-house Skill chain) → R25-mid Codex audit (post-Phase 2 Session 4, anchor `87c2ba2`) verdict GO-WITH-CAVEATS 5 P0 → **V0.14 (R25-mid Codex audit P0 fix; v7.1 reframe)** → R25-final (planned, post-Phase 2 Session 8)
